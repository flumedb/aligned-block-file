  ...
  return self = {
    get: function (i, cb) {
      offset.once(function (_offset) {
        var max = Math.floor(_offset / block_size)
    ...
    append: function (buf, cb) {
      ...
      fd.once(function (_fd) {
        if (typeof _fd === 'object') { return cb(_fd) }
        offset.once(function (_offset) {
    ...
    write: function (buf, writeOffset, cb) {
      if (writing++) throw new Error('already writing to this file')
      fd.once(function (_fd) {
        // Not sure, but something like "if this isn't a file descriptor"
        if (typeof _fd === 'object') return cb(_fd)

        fs.write(_fd, buf, 0, buf.length, writeOffset, function (err, written) {
          writing = 0
          if (err) return cb(err)
          if (written !== buf.length)
            return cb(new Error('wrote less bytes than expected:' + written + ', but wanted:' + buf.length))
          cb(null)
        })
      })
    },
