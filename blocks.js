...
function assertInteger (a) {
  if (!Number.isInteger(a)) { throw new Error('expected positive integer, was:' + JSON.stringify(a)) }
}

var Cache = require('hashlru')

module.exports = function (file, block_size, cache) {
  var cbs = []; var br; var writing = 0
  cache = cache || Cache(1000)

  function get (i, cb) {
    var c = cache.get(i)
    if (Buffer.isBuffer(c)) { cb(null, c, block_size) } else if (Array.isArray(cbs[i])) { cbs[i].push(cb) } else {
      cbs[i] = [cb]
      file.get(i, function (err, buf, bytes_read) {
        var cb = cbs[i]
        cbs[i] = null
        if (!err) cache.set(i, buf)
        for (var j = 0; j < cb.length; ++j) { cb[j](err, buf, bytes_read) }
      })
    }
  }
  ...
  function read (start, end, cb) {
    ...
    if (file && end > file.offset.value) { return cb(new Error('past end:' + start + '-' + end + ' < ' + file.offset.value), null, 0) }
    var bufs = []
    ;(function next (i) {
        ...
        function next () {
          while (b_start < buf.length) { // start < _offset+buf.length) {
            ...
            if (Buffer.isBuffer(b)) {
              ...
              start += len
              b_start += len
            } else if (Array.isArray(cbs[i])) { throw new Error('should never happen: new block should be initialized, before a read ever happens') } else {
              start += block_size
            }
